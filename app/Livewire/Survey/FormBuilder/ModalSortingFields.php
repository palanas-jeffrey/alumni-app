...
class ModalSortingFields extends Component
{
    ...
    public function mount( int $section_id = null)
    {
        ...
        $this->items = SurveySectionField::where('section_id', $this->section_id)->orderBy('order')->get();
    ...
    public function saveOrder()
    {
        foreach ($this->stagedOrder as $order) {
            SurveySectionField::where('id', (int)$order['value'])->update(['order' => $order['order'] + 1]);
        }
    ...
    {
        return view('livewire.survey.form-builder.modal-sorting-fields');
    }
}
