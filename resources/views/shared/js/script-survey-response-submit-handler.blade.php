<script>
   var isResponseUpdate = @json($isResponseUpdate ?? false);

    async function submitResponse() {
        const formContainer = document.querySelector("#survey-form");
        ...
        try {
            ...
            setTimeout(() => {
                if (isResponseUpdate) {
                    ...
                    window.location.replace("{{ route('survey-completion') }}");
                }
            }, 1500);

        } catch (error) {
            showToast("Encountered error while saving the response: " + error.message, false);
        }
    }
    ...
    async function isUploadDocumentSuccess(inputElement, count = 0, totalFiles = 0) {
        ...
        try {
            const response = await fetch("{{ route('survey.save-documents', ['form_id' => $form_id]) }}", {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': token
                },
