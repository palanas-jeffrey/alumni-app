<x-generic-layout>
    <x-simple-nav>
        <div class="d-flex">
            <div class="absolute font-semibold leading-tight text-gray-800 text-xl">
                <a href="{{ route('tracer.report-overview.batches', ['form_id' => $form->id, 'program_id' => $program_id]) }}">
            ...
            <div class="poppins-semibold text-center text-xl w-100">
                <h1>
                    Tracer study response analysis
                </h1>
            </div>
        </div>
    </x-simple-nav>
        ...
        <div class="tracer-program-overview max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    @if($sections)
                        @foreach($sections as $section)
                            <div class="pb-4 pt-4">
                                ...
                                @livewire("tracer.report.section-response-analysis", 
                                    ['section_id' => $section->id, 'program_id' => $program->id,
                                     'form_id' => $form->id, 'batch_year' => $batch_year])
                            </div>
                        @endforeach
                    @else
            ...
            <div class="flex-grow-1">
                <div class="bg-dust-gray mb-3 p-4 rounded-4">
                    <div>
                        <h2>
                            <h2 id="modalHeadingtxt" class="font-medium mb-3 poppins-semibold text-lg">Generate tracer report</h2>
                        </h2>
                        <div>
                            <x-primary-button type="button"
                                data-bs-toggle="modal" data-bs-target="#generateTracerReport">Generate</x-primary-button>
                        </div>
                    </div>
                </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="text-right">
                                                        <a href="{{ route('tracer.view-response', ['response_id' => $response->id]) }}">
    ...
    <x-modal-generic :modalId="'generateTracerReport'">
        <div class="modal-body p-4 pt-0">
            <div class="space-y-6">
                <h2 id="modalHeadingtxt" class="modal-title poppins-semibold text-xl">Would you like to generate tracer reports?</h2>
            </div>
        </div>

        <form action="{{ route('tracer.generate-report') }}" method="POST"  target="_blank">
            @csrf
            <div>
