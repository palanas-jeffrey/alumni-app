<div class="pb-2 text-right">
    @if(!$isModalOnly)
        <x-primary-button type="button" x-data=""
            x-on:click.prevent="$dispatch('open-modal', 'modal-clone-survey-form-{{ $form_id }}')">Clone survey form</x-primary-button>
    @endif
    ...
    <x-modal name="modal-clone-survey-form-{{ $form_id }}"
        ...
        @if ($new_form_id)
            <div class="p-4 pb-0 text-left">
                <h2 class="mb-1 modal-title poppins-semibold text-xl">
                    Would you like to see the overview?
                </h2>
            </div>
            <div class="flex justify-start p-4">
                <x-link-btn href="{{ route('survey.survey-form-overview', $new_form_id) }}">View</x-link-btn>
        ...
        @else
            <div class="p-4 pb-0 text-left">
                <h2 class="mb-1 modal-title poppins-semibold text-xl">
                    Would you like to clone the survey form?
                </h2>
            </div>
            <div class="flex justify-start p-4">
